...
uint8_t i2c_write_write(const uint8_t reg_addr, uint8_t* const bytes, const uint16_t byte_count)
{
	I2C_Start();	          
	...
	for (int16_t i = 0; i < byte_count; i++) {
...
uint8_t i2c_write_read(const uint8_t reg_addr, uint8_t* const bytes, const uint16_t byte_count)
{
	...
	bytes[byte_index] = I2C1->D;
	I2C_Wait();

	for(int16_t byte_index = 0; byte_index < (byte_count - 2); byte_index++)
	{
		bytes[byte_index] = I2C1->D;
		I2C_Wait();
	}
